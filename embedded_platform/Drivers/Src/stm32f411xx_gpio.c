...
void GPIO_ClkCtrl(GPIO_RegDef_t *pGPIOx, uint8_t status) {
	if(status == ENABLE) {
		if(pGPIOx == GPIOA) GPIOA_EN_CLK();
		else if(pGPIOx == GPIOB) GPIOB_EN_CLK();
		else if(pGPIOx == GPIOC) GPIOC_EN_CLK();
		else if(pGPIOx == GPIOD) GPIOD_EN_CLK();
		else if(pGPIOx == GPIOE) GPIOE_EN_CLK();
		else if(pGPIOx == GPIOH) GPIOH_EN_CLK();

	}else if(status == DISABLE){
		if(pGPIOx == GPIOA) GPIOA_DIS_CLK();
		else if(pGPIOx == GPIOB) GPIOB_DIS_CLK();
		else if(pGPIOx == GPIOC) GPIOC_DIS_CLK();
		else if(pGPIOx == GPIOD) GPIOD_DIS_CLK();
		else if(pGPIOx == GPIOE) GPIOE_DIS_CLK();
		else if(pGPIOx == GPIOH) GPIOH_DIS_CLK();
	}
}
...
void GPIO_Init(GPIO_Handle_t * pGPIOHandle) {
	...
	//2. Configurar modo (IN, OUT, AF, AN)
	if(pGPIOHandle->GPIO_config.GPIO_Mode >= GPIO_MODE_IT_RE && pGPIOHandle->GPIO_config.GPIO_Mode <= GPIO_MODE_IT_RFE) {
	...
	//6. Configurar función alternativa
...
void GPIO_IRQHandler(uint8_t pin){
	if(EXTI->PR & (1 << pin)) EXTI->PR |= (1 << pin);
}
